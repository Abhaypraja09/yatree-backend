...
const getDashboardStats = asyncHandler(async (req, res) => {
    ...
    try {
        ...
        const [
            ...
            Promise.all([
                Vehicle.aggregate([{ $match: { company: companyObjectId, isOutsideCar: true } }, { $project: { month: { $substr: [{ $ifNull: ["$carNumber", ""] }, { $add: [{ $indexOfBytes: ["$carNumber", "#"] }, 1] }, 7] }, isBuy: { $eq: [{ $ifNull: ["$transactionType", "Buy"] }, "Buy"] }, amount: { $cond: { if: { $and: [{ $eq: [{ $ifNull: ["$transactionType", "Buy"] }, "Buy"] }, { $gt: [{ $ifNull: ["$buyAmount", 0] }, 0] }] }, then: "$buyAmount", else: { $ifNull: ["$dutyAmount", 0] } } }, isE: { $gt: [{ $strLenCP: { $toString: { $ifNull: ["$eventId", ""] } } }, 10] } } }, { $facet: { e: [{ $match: { month: monthPrefix, isE: true } }, { $group: { _id: null, t: { $sum: "$amount" } } }], o: [{ $match: { month: monthPrefix, isE: false, isBuy: true } }, { $group: { _id: null, t: { $sum: "$amount" } } }] } }]),
