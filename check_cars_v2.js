...
const connectDB = async () => {
    try {
        ...
        const cars = await Vehicle.find({ company: '6982e8b7d0b069a49db197b9' });
        console.log('Total Cars for 6982:', cars.length);
        cars.forEach(c => console.log(`Num: ${c.carNumber}, Status: ${c.status}, Outside: ${c.isOutsideCar}`));
        process.exit();
    } catch (err) {
        process.exit(1);
    }
