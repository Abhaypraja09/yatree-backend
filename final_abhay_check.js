...
const run = async () => {
    try {
        ...
        const advs = await mongoose.connection.db.collection('advances').find({ driver: abhayId }).toArray();
        console.log(`Abhay (${abhayId}) has ${advs.length} advances in collection.`);
        advs.forEach(a => {
